...
class Listas_embalaje_model extends CI_Model {
    ...
    function getEncuestasExportar($datos, $ocultarCampos){
        ...
        $this->db->start_cache();
        $this->db->select('fecha_tamizacion as fecha_toma, cedula as numero_documento, municipio');
        $this->db->from('pin_tamizadasf4');
        $this->db->where('municipio',$datos->municipio);
        $this->db->where('fecha_tamizacion',$datos->fechaToma);
        $this->db->where('ips',$datos->ips);
        //$this->db->where('ips',$datos->ips);
    ...
    function getEncuestasExportar_1($datos, $ocultarCampos) {
        $inferior = (($datos->current - 1) * $datos->rowCount);
        $superior = $datos->rowCount;
        $fechaToma = $datos->fechaToma;
    ...
    inner join par_ips on resultado_epidemiologica.ips = par_ips.nombre_ips
    where pin_toma_citologia.fecha_toma = \'' . $fechaToma . '\'';
        if ($datos->searchPhrase !== '') {
        ...
        $sql = $sql . 'where consulta.id_ips  = \'' . $datos->ips . '\'  and municipio =\'' . $datos->municipio . '\' ';
    ...
    function getIpsbyId($id_ips) {
        $this->db->select('id_ips, nombre_ips');
        $this->db->from('par_ips');
        $this->db->where('id_ips', $id_ips);
        $consulta = $this->db->get();
        if ($consulta) {
