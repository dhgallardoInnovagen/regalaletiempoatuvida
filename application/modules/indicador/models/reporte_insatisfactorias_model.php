...
class Reporte_insatisfactorias_model extends CI_Model {
    ...
    function getReporteInsatisfactorias($datos) {
        $inferior = (($datos->current - 1) * $datos->rowCount);
        $superior = $datos->rowCount;
        $this->db->start_cache();
        $this->db->distinct();
        $this->db->from('pin_insatisfactorias');
        if ($datos->searchPhrase !== '') {
            $this->db->or_where("cedula like ('%" . $datos->searchPhrase . "%')");
            ...
            $this->db->or_where("UPPER(campania) like UPPER('%" . $datos->searchPhrase . "%')");
            $this->db->or_where("UPPER(cod_mx) like UPPER('%" . $datos->searchPhrase . "%')");
        ...
        $rows["rows"] = $arrayResultadoInicial;
...
//Fin function
    ...
    function getReporteTamizacionExportar() {
        ...
        $this->db->join('resultado_epidemiologica', 'resultado_epidemiologica.numero_documento = pin_resultado_laboratorio.cedula  ');
                ...
                if ($row->calidad_muestra == 'Satisfactoria con endocervicales/zona de transformación' || $row->calidad_muestra == 'Satisfactoria sin endocervicales/zona de transformación') {//2
                    if ($row->categorizacion_general == 'Anormalidades en células epiteliales') {//Inicio if Categorizacion general
                        ...
                        if ($row->categorizacion_general == 'Negativa para lesión intraepitelial o malignidad') {
                            $row->fecha_toma_citologia = 'Negativo';
                        }
                    }
                }
                unset($row->ips);
